...
variable "use_pocketbase" {
  type = bool
  description = "Enable PocketBase"
  default = false
  sensitive = false
}
...
resource "coder_agent" "main" {
  ...
  os             = "linux"
  startup_script = <<EOT
  use_pocketbase="${var.use_pocketbase ? "y" : "n"}"

  # Install parcel template project
  if [ ! -d "/home/coder/project" ]
  then
    mkdir -p /home/coder/project
  fi

  # Install pocketbase
  if [ ! -d "/home/coder/db" ] && [ "$use_pocketbase" == "y" ]
  then
  ...
  if [ "$use_pocketbase" == "y" ]
  then
    /home/coder/db/pocketbase serve --dir /home/coder/db/pb_data --publicDir /home/coder/project/www/dist &
  fi
...
resource "coder_app" "pocketbase" {
  count = var.use_pocketbase ? 1 : 0
  agent_id     = coder_agent.main.id
  slug         = "pocketbase"
